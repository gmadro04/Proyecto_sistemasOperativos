...
def main(args=None):
    ...
    # --- CODIGO FMS CON AISLAMIENTO SCHED_FIFO ---
    try:
        pid = os.getpid()
        prioridad_rt = 50  # Rango en Linux 1 a 99 donde 99 es la prioridad mas alta absoluta
        
        # Configurar los parámetros de planificación
        param = os.sched_param(prioridad_rt)
        
        # Solicitar al kernel el cambio a SCHED_FIFO
        os.sched_setscheduler(pid, os.SCHED_FIFO, param)
        print(f"[RT-KERNEL] Política SCHED_FIFO tiempo Real aplicada con éxito al PID {pid}. Prioridad: {prioridad_rt}")
        
    except PermissionError:
        print("[WARNING] Permisos denegados para SCHED_FIFO. El nodo correrá bajo CFS.")
        print("          -> Solución: Ejecuta el nodo con 'sudo' o configura /etc/security/limits.conf")
    except AttributeError:
        print("[WARNING] El sistema operativo actual no soporta os.sched_setscheduler (Usa Linux).")
    # ----------------------------------------------

    node = QupaExperimentNode()
    try:
        # duerme entre los callbacks del timer a 5Hz, cediendo la CPU de raspberry pi.
        rclpy.spin(node)
